...
def svm_loss(x, y):
    ...
    N=x.shape[0]
    correct=x[range(N),y].reshape(-1,1)
    margin=np.maximum(0,x-correct+1)
    loss = (np.sum(margin)/N)-1
    
    pos_mar=np.zeros(margin.shape)
    pos_mar[margin > 0]=1
    pos_mar[range(N),y]=0
    pos_mar[range(N), y]=-np.sum(pos_mar, axis=1)
